        ...
        implements View.OnCreateContextMenuListener, MusicUtils.Defs, ServiceConnection
{
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        /*
         * SLSI_ULP_AUDIO: ULP Bypass
         * Make menu button for ULP Bypass
         */
        menu.add(0, ULP_BYPASS, 0, R.string.ulp_bypass).setIcon(R.drawable.ic_menu_set_as_ringtone);

        /*
         * SLSI_ULP_AUDIO: Sound Alive on/off
         * Make menu button for control of sound alive
         */
        menu.add(0, SA_CTRL, 0, R.string.sa_ctrl).setIcon(R.drawable.ic_menu_set_as_ringtone);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        Cursor cursor;
        switch (item.getItemId()) {
            /* SLSI_ULP_AUDIO: Sound Alive control */
            case SA_CTRL:
                intent = new Intent();
                intent.setClass(this, SetSAControl.class);
                startActivityForResult(intent, SA_CTRL);
            break;
            /* SLSI_ULP_AUDIO: ULP Bypass */
            case ULP_BYPASS:
                intent = new Intent();
                intent.setClass(this, SetUlpBypass.class);
                startActivityForResult(intent, ULP_BYPASS);
            break;
        ...
        switch (requestCode) {
            ...
            case ULP_BYPASS:
                if (resultCode == RESULT_OK) {
                    boolean ulpBypassOn = intent.getBooleanExtra("com.android.music.ulp_bypass", false);

                    try {
                        MusicUtils.sService.ulpBypass(ulpBypassOn);

                        long [] list = MusicUtils.sService.getQueue();
                        int index = MusicUtils.sService.getQueuePosition();
                        long pos = MusicUtils.sService.position();

                        MusicUtils.sService.stop();
                        MusicUtils.sService.open(list, index);
                        MusicUtils.sService.seek(pos);
                        MusicUtils.sService.play();
                    } catch (RemoteException ex) {
                        Log.e(LOGTAG, "ULP Bypass error!");
